...
- **38 — Opportunity Quick View Tab & Modal (`OpportunityQuickViewModal.tsx`)**: Dedicated top navigation tab positioned right before the *Deleted* tab across Lead Pool (`/`), Lead Pool page (`/lead-pool`), and Inbound Leads (`/new-leads`). Gives bosses, managers, and agents single-click access to filter leads with active opportunities and launch a high-density deal preview modal containing pipeline stage, deal value range, developer/project specs, qualification preferences, SLA status, notes, and direct action triggers.
- **39 — Dynamic Roles CRUD, Departments CRUD & Per-User Auto-Lead Distribution Switch**: Per-user **`Include in Auto-Lead Distribution`** toggle (`in_distribution_pool`) in User Management (`/users`). Turning OFF allows auditors, reviewers, and testing accounts to log in and inspect the system without receiving auto-assigned leads or 3-day/45-day SLA rotations. Includes 100% dynamic CRUD for company **Departments** (`/api/departments`) and dynamic **Roles** with customizable module permission matrices.
- **40 — Role & User Permissions Modal Unification & Instant Bidirectional Sync**: 100% unified design, `max-w-4xl` layout width, section styling, and rich permission cards (title, key, and detailed description `desc`) between **Edit Role Details** modal and **Granular CRM Permissions** modal. Automatically expands wildcard (`*`) permissions, adds global + per-module **`Select All`** / **`Deselect All`** toggles, and implements full bidirectional synchronization: updating permissions on a user automatically updates the parent Role and peer users, and updating a Role immediately propagates to all assigned users and refreshes frontend states without manual reloads (`UserController.php`, `RoleController.php`).
...
- **42 — WhatsApp Dynamic Channel Resolution, Multi-Device Auto-Pairing & Resilient Chat Ingestion**:
  - **Dynamic Channel Resolution**: Fixed rigid hardcoded `channel_id: 1` calls across `whatsapp-gateway/server.js` (`pair-confirm`, `disconnect`, `webhook`, `sync-phone-data`). Live MySQL databases frequently begin channel IDs higher (e.g. ID 5–10). Updated endpoints to dynamically resolve the channel via `active`, matching telephone number, authenticated user channel, or agency master channel.
  - **Reconciliation & False Disconnection Removal**: Eliminated erroneous false disconnection logic in `WhatsAppController::generateQr` where checking uninitialized channel phone numbers caused newly linked channels to immediately revert to `status = 'disconnected'`. Channel records now proactively reconcile with live gateway state.
  - **Immediate Pair-Confirm & Auto Sync on QR Scan**: In `frontend/src/app/whatsapp/page.tsx`, the QR scanning poller now immediately issues `POST /api/whatsapp/channels/{targetId}/pair-confirm` with the scanned phone number upon connection and invokes `POST /api/sync` on the gateway daemon. The header banner switches smoothly to `🟢 Online` without displaying `DISCONNECTED - CLICK TO LINK`.
  - **Modern History Ingestion via Official `client.getChats()`**: Replaced broken internal prototype DOM extraction (`window.require('WAWebCollections')`, deprecated by recent WhatsApp Web updates) with the official, reliable `client.getChats()` API in `whatsapp-web.js`, accompanied by a robust `window.Store` fallback. Ensures genuine 1-on-1 contact chats, timestamps, and message history are extracted and synced to Laravel without silent failures.
  - **Dynamic Production Backend API Auto-Discovery (`postToLaravel`)**: Replaced previously hardcoded `127.0.0.1:8000` URLs in the gateway daemon with a resilient multi-target resolver (`https://api.fsadvisory.ae`, `http://127.0.0.1`, local ports). Resolves the production backend connection refused bug (`ECONNREFUSED 127.0.0.1:8000`), allowing both history sync batches and incoming webhooks to reach MySQL instantly.
  - **Real-Time Inbound Message Streaming (No Page Reload)**: Inbound message webhooks arriving at the gateway are immediately dispatched to `https://api.fsadvisory.ae/api/whatsapp/webhook`, updating the conversation in the database. The frontend silent poller automatically renders the incoming chat bubble and smoothly scrolls to the bottom within 2 seconds without requiring manual page reloads.
  - **Automatic LID Resolution & Duplicate Chat Auto-Merge**: Fixed multi-device privacy format issue where incoming WhatsApp Web messages from `@lid` JIDs spawned disconnected duplicate threads (`[numbers]@lid`) rather than routing to the real contact's chat. The gateway daemon now extracts the genuine international phone number via `msg.getContact()`, while `WhatsAppController::webhook` performs global phone/remote_jid matching across all channels and `WhatsAppController::chats` automatically heals and merges orphaned LID messages into the true contact conversation.
  - **Agent Chat Query Ingestion**: Enhanced `WhatsAppController::chats()` and `syncPhoneData()` so non-super-admin advisors can access synced chats and contacts under the agency's connected channel seamlessly.
