...
- **40 — Permission Checkboxes & Wildcard Permission Expansion**: Fully interactive permission matrix in both **Edit Role Details** modal and **Granular CRM Permissions** modal. Automatically expands wildcard (`*`) permissions into explicit keys upon modal launch, and adds global + per-module **`Select All`** / **`Deselect All`** toggles. Streamlined table row actions by eliminating duplicate per-row buttons in favor of top tab navigation.
