...
- **40 — Role & User Permissions Modal Unification & Instant Inheritance Sync**: 100% unified design, `max-w-4xl` layout width, section styling, and rich permission cards (title, key, and detailed description `desc`) between **Edit Role Details** modal and **Granular CRM Permissions** modal. Automatically expands wildcard (`*`) permissions, adds global + per-module **`Select All`** / **`Deselect All`** toggles, and synchronizes updated role permissions to all assigned users in real-time (`RoleController.php`).
